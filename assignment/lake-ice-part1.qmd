...
ice_data <- read_excel(file_name, sheet = "citation")
...
ggplot(sunapee, aes(x = year, y = ice_off_doy))+
  geom_point() +
  geom_line() +
  labs(title = "Lake Sunapee ice off day over time",
       x = "year", y = "ice off day") 
...
I expect for the sign of the slope to be negative because ice_off_doy represents the day in the year
when the ice melts and the climate is always getting warmer over year. That means as the years increase, 
the ice_doy_off should decrease which would lead to a negative relationship.
...
**Answer 3:**
The direction of the slope is negative, which did match my expectations from question 2. Because the climate
is always getting warmer, that means the ice would be melting a lot more and earlier. That also means that 
as the years increase, the ice_off_doy would decrease.
...
The R2 for the regression is 0.2055 which means that about 20.55% is of the variation is explained.
This shows that the relation is significant but not very strongly related.
...
**Answer 6:**
The residuals do meet the assumptions of the linear regression as they are normally distributed
centered around zero. 

```{r}
ggplot(sunapee, aes(x = residual)) +
  geom_histogram(bins = 30, color = "black", fill = "lightblue4")
...
**Answer 8:**
Based on the slope value, around 66 years are needed to pass before the lake has a 1 week change. I think that
this is meaningful because it shows how over the years things slowly change.
...
I am decently comfortable with using this model for additional analysis. I think that the data makes sense and the visual representations from the histogram and graphs as well really help see that. 
...
```{r}
#ice-off day of year 2040

predict_2040 <- (-0.1060227 * 2040) + 318.9485608
  print(predict_2040)
#Answer = 102.6623

#90% confidence interval

lower_interval <- quantile(sunapee$residual, 0.05)
upper_interval <- quantile(sunapee$residual, 0.95)

lower_interval
upper_interval

#90% confidence interval with 2040 prediction

lower_ci <- predict_2040 + lower_interval
upper_ci <- predict_2040 + upper_interval
lower_ci 
upper_ci
# 95% = 116.8138, 5% = 82.23494
...
early_period <- sunapee |> 
  filter(year < 1970)

later_period <- sunapee |> 
  filter(year >= 1970)

fit_early <- lm(ice_off_doy ~ year, data = early_period)
fit_late  <- lm(ice_off_doy ~ year, data = later_period)

combined <- rbind(
  early_period |> mutate(period = "Before 1970"),
  later_period |> mutate(period = "1970 and after")
)

# Plot
ggplot(combined, aes(x = year, y = ice_off_doy, color = period)) +
  geom_point(alpha = 0.6) +
  geom_line() +
  labs(
    title = "Lake Sunapee Ice-Off Day Over Time",
    x = "Year",
    y = "Ice-Off Day of Year"
  ) 
```
The ice-off day of year is changing more quickly is changing more quickly in recent years. Initially to divide the data, I filtered the sunapee data set to be either "<1970" or ">=1970) and then from there, i made linear models to see the data from the 2 separate periods. From looking at the data, you can see that for the earlier time period the slope is -0.109668 and the later period is -0.3023923. That means ice-off day is changing about 0.11 earlier days per year for the earlier period, while its changing about 0.30 days earlier per year for the later period. In recent times, change has been happening a lot more rapidly, which explains the larger, steeper negative slope.
